...
Diorama.prototype.draw = function () {
    ...
    this.cutouts.map(cutout => {
        if (this.tiltable) {
            this.tilt.x = this.tilt.x >= this.tilt.limits.x[0] ? this.tilt.x : this.tilt.limits.x[0]
            this.tilt.x = this.tilt.x <= this.tilt.limits.x[1] ? this.tilt.x : this.tilt.limits.x[1]
            this.tilt.y = this.tilt.y >= this.tilt.limits.y[0] ? this.tilt.y : this.tilt.limits.y[0]
            this.tilt.y = this.tilt.y <= this.tilt.limits.y[1] ? this.tilt.y : this.tilt.limits.y[1]

            document.getElementsByClassName('show-title')[0].innerText = this.tilt.x + "|" + this.tiltMagnitude + "|" + (this.tilt.x / Math.abs(this.tiltLimits.x[0] - this.tiltLimits.x[1])) + "|" + (this.tilt.y / Math.abs(this.tiltLimits.y[0] - this.tiltLimits.y[1]));

            let adjustedTilt = {
                x: this.tilt.x * this.tiltMagnitude * (this.tilt.x / Math.abs(this.tiltLimits.x[0] - this.tiltLimits.x[1])),
                y: this.tilt.y * this.tiltMagnitude * (this.tilt.y / Math.abs(this.tiltLimits.y[0] - this.tiltLimits.y[1])),
            }

            this.ctx.drawImage(
                cutout.image,
                (cutout.anchor.x - (this.position.x + this.tilt.x)) * ((this.limits.z[0] - cutout.anchor.z) / this.limits.z[0]),
                (cutout.anchor.y - (this.position.y + this.tilt.y) * (this.limits.z[0] - cutout.anchor.z) / this.limits.z[0]) - (cutout.image.height * this.scale) + canvas.height,
