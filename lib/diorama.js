...
Diorama.prototype.draw = function () {
    ...
    this.cutouts.map(cutout => {
        if (this.tiltable) {
            ...
            let adjustedTilt = {
                x: this.tiltMagnitude * (this.tilt.x / Math.abs(this.tiltLimits.x[0] - this.tiltLimits.x[0])),
                y: this.tiltMagnitude * (this.tilt.y / Math.abs(this.tiltLimits.y[0] - this.tiltLimits.y[0])),
            }
            ...
            this.ctx.drawImage(
                cutout.image,
                (cutout.anchor.x - (this.position.x + adjustedTilt.x)) * ((this.limits.z[0] - cutout.anchor.z) / this.limits.z[0]),
                (cutout.anchor.y - (this.position.y + adjustedTilt.y) * (this.limits.z[0] - cutout.anchor.z) / this.limits.z[0]) - (cutout.image.height * this.scale) + canvas.height,
